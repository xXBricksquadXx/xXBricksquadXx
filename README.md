<!-- Profile README for github.com/xXBricksquadXx -->

<h1 align="center">Hi, I'm Colby (xXBricksquadXx)</h1>

<p align="center">
  <b>Data tools · Web apps · Clean starters</b><br/>
  Pulaski, TN · Building small, useful things and shipping often.
</p>

<p align="center">
  <a href="https://rosehillops.com/">🌐 rosehillops.com</a> ·
  <a href="https://github.com/xXBricksquadXx?tab=repositories">📂 Repos</a> ·
  <a href="https://dev.to/xxbricksquadxx">✍️ Dev articles</a>
</p>

---

<!-- Typing animation -->
<p align="center">
  <a href="https://github.com/xXBricksquadXx">
    <img
      src="https://readme-typing-svg.demolab.com?size=22&center=true&vCenter=true&width=650&lines=Ship+small%2C+useful+tools;Data+%26+analytics+with+Python+%2B+DuckDB;Next.js+%2B+TypeScript+frontends;Clean+starters+for+future+me"
      alt="Typing SVG"
    />
  </a>
</p>

---

### 🧭 What I'm focused on

- 🐍 **Python + DuckDB + Streamlit** for quick data tools  
- 📊 **Plotly Dash & notebooks** for analysis and dashboards  
- ⚡ **Next.js 14 + TypeScript + Tailwind** for small productized sites  
- 📄 **PDF / document utilities** (signing, conversion, micro-workflows)

---

### 🚀 Flagship / product-style projects

- **CSV Studio — DuckDB Edition**  
  Upload CSV/XLSX → filter, chart, CRUD, export — all in the browser.  
  `Python · Streamlit · DuckDB`  
  👉 <https://github.com/xXBricksquadXx/csv-studio-duckdb>

- **GitHub Repo Health Dash**  
  Plotly Dash dashboard that inspects recent commit history of any public repo.  
  `Python · Dash · GitHub API`  
  👉 <https://github.com/xXBricksquadXx/github-repo-health-dash>

- **Farmer's Almanac (TN-centric)**  
  Next.js almanac UI with seasonal focus and clean layout.  
  `Next.js · TypeScript · Tailwind · Vercel`  
  👉 <https://github.com/xXBricksquadXx/farmers-almanac>

- **PDF Signer (Vue)**  
  Drop a PDF, place a signature, download the signed copy.  
  `Vue · Vite`  
  👉 <https://github.com/xXBricksquadXx/pdf-signer-vue>

- **Next.js Clean Starter**  
  Opinionated Next.js 14 + Tailwind starter with Airbnb ESLint + Prettier.  
  `Next.js · TypeScript · Tailwind · ESLint · Prettier`  
  👉 <https://github.com/xXBricksquadXx/nextjs-clean-starter>

---

### 📊 GitHub stats & language graph

<p align="center">
  <img
    src="https://github-readme-stats.vercel.app/api?username=xXBricksquadXx&show_icons=true&hide_title=true&hide_border=true&count_private=true&include_all_commits=true"
    alt="Colby's GitHub stats"
  />
</p>

<p align="center">
  <img
    src="https://github-readme-stats.vercel.app/api/top-langs/?username=xXBricksquadXx&layout=compact&hide_border=true&langs_count=8"
    alt="Top languages"
  />
</p>

<p align="center">
  <img
    src="https://github-readme-activity-graph.vercel.app/graph?username=xXBricksquadXx&hide_border=true"
    alt="Contribution graph"
  />
</p>

---

### 🧰 Toolbox

**Languages**

- Python · TypeScript · JavaScript  
- SQL · Bash/Batch · a little HTML/CSS when needed

**Data / analytics**

- DuckDB · pandas · Plotly · Jupyter  
- Streamlit · Dash

**Web / frontend**

- Next.js (App Router) · React  
- Vue · Tailwind CSS

**Dev workflow**

- GitHub Actions · Vercel · Docker (where it helps)  
- Opinionated linters/formatters (Airbnb ESLint, Prettier)

---

### 📚 Selected analysis work

- **TN Unemployment Analysis** – time-series and regional patterns for Tennessee.  
  👉 <https://github.com/xXBricksquadXx/TN-unemployment-analysis>

- **Chicago Data Analysis** – crime, census, and school data mashup.  
  👉 <https://github.com/xXBricksquadXx/ChicagoDataAnalysis>

---

### 📝 Writing & notes

I also write about what I’m building and learning:

- Dev.to: <https://dev.to/xxbricksquadxx>

If a repo looks like a “starter” or “lab,” it probably is: feel free to fork, adapt, and ship.

---

<p align="center">
  <i>If it didn’t demo, it didn’t happen.</i>
</p>
